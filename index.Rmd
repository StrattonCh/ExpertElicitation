...
sim_idyllic <- function(n = 100, J = 8, seed = 1, beta = c(0,1), p = .5, tau = 1, nregions = 4, root = 3){
  ...
  out <- list(
    df = grid,
    params = list(
      beta = beta, eta = grid$eta, theta = c(theta), p = p, pi = grid$pi, z = grid$z, tau = tau
    ),
...
summary(stan_fit)$summary[1:9,c(1, 4, 8:10)]
...
tbl1 <- tibble(
  ...
  lwr = tmp[,2],
  upr = tmp[,3],
  truth = with(sim_dat$params, c(beta, theta, p, eta))
) %>%
  mutate(
